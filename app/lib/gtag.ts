 ...
 * Privacy Settings, without a page reload). There's no way to change which
 * ID a `<script>` tag was loaded with after the fact, so this issues a
 * `config` call for the Ads ID on the existing (GA4-bootstrapped) gtag.js
 * runtime — the same `config` mechanism used on the first-load path in
 * syncGoogleConsent below, which is what actually registers a destination
 * for event routing (see the file-level comment).
 */
 ...
 * then explicitly `config` it (the `config` call is what actually
 * registers it as a routable destination — see the file-level comment;
 * the script `id=` alone does not), then also `config` GA4 if Analytics
 * is granted too. Advertising absent, Analytics present → bootstrap with
 * the GA4 ID only, exactly as before; Google Ads is never loaded or
